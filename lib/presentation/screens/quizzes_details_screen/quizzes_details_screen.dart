...
class QuizzesDetailsScreen extends StatelessWidget {
  const QuizzesDetailsScreen({super.key, required this.quizzes});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<QuizzesCubit,QuizzesStates>(
      builder: (context, state) => Scaffold(
        ...
        body: PageView(
          children: [
            QuizDayQuestions(dayLocalize: "friday",function:(parentIndex, childIndex) => cubit.updateQuizAnswers(parentIndex, childIndex, "Friday"),
              questions: quizzes.friday?.questions??[],
              shahid: quizzes.friday?.shahid??"",
              answers: cubit.quizAnswers.friday?.answers?? [],
            ),
            QuizDayQuestions(dayLocalize: "saturday",function:(parentIndex, childIndex) => cubit.updateQuizAnswers(parentIndex, childIndex, "Saturday"),
              questions: quizzes.saturday?.questions??[],
              shahid: quizzes.saturday?.shahid??"",
              answers: cubit.quizAnswers.saturday?.answers?? [],
            ),
            QuizDayQuestions(dayLocalize: "sunday",function:(parentIndex, childIndex) => cubit.updateQuizAnswers(parentIndex, childIndex, "Sunday"),
              questions: quizzes.sunday?.questions??[],
              shahid: quizzes.sunday?.shahid??"",
              answers: cubit.quizAnswers.sunday?.answers?? [],
            ),
            QuizDayQuestions(dayLocalize: "monday",function:(parentIndex, childIndex) => cubit.updateQuizAnswers(parentIndex, childIndex, "Monday"),
              questions: quizzes.monday?.questions??[],
              shahid: quizzes.monday?.shahid??"",
              answers: cubit.quizAnswers.monday?.answers?? [],
            ),
            QuizDayQuestions(dayLocalize: "tuesday",function:(parentIndex, childIndex) => cubit.updateQuizAnswers(parentIndex, childIndex, "Tuesday"),
              questions: quizzes.tuesday?.questions??[],
              shahid: quizzes.tuesday?.shahid??"",
              answers: cubit.quizAnswers.tuesday?.answers?? [],
            ),
            QuizDayQuestions(dayLocalize: "wednesday",function:(parentIndex, childIndex) => cubit.updateQuizAnswers(parentIndex, childIndex, "Wednesday"),
              questions: quizzes.wednesday?.questions??[],
              shahid: quizzes.wednesday?.shahid??"",
              answers: cubit.quizAnswers.wednesday?.answers?? [],
            ),
            QuizDayQuestions(dayLocalize: "thursday",function:(parentIndex, childIndex) => cubit.updateQuizAnswers(parentIndex, childIndex, "Thursday"),
      ...
      listenWhen: (previous, current) => current is OnGetBack || current is OnSubmit,
    );
  }
}
